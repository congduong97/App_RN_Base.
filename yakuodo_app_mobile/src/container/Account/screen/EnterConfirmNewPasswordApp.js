...
export default class EnterConfirmNewPasswordApp extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            listTextInput: ['', '', '', ''],
    ...
    changeDataParent = (index, value) => {
        ...
        if(check){
            const {password,screenName} = this.props.navigation.state.params
            if(pass===password){
                
                managerAcount.passwordApp = pass
                AsyncStorage.setItem(keyAsyncStorage.managerAccount,JSON.stringify(managerAcount))
                this.props.navigation.navigate(screenName)
                Alert.alert('パスコードを変更しました。')
            }else{
                this.setState({textError:'パスコードが一致しません。新しいパスコードを再度ご入力ください。'})
            }
        }
    }
    goBack=()=>{
    ...
    renderInput = () => {
        ...
        const listInput = listTextInput.map((value, index) => <InputPass
            nameScreen={'EnterConfirmNewPasswordApp'}
            widthInput={30}
            marginCenter
        ...
        return (
            <View style={{ flex: 1, alignItems: 'center', backgroundColor: COLOR_WHITE }}>
                ...
                <KeyboardAwareScrollView
                    ...
                    style={{ flex: 1 }}
                >
                    <Text style={{ marginVertical: 25, fontSize: 16 }}> {'パスコードを再度入力してください'} </Text>
                    {this.renderInput()}
