...
const Api = {
  ...
  //Đăng kí hoặc update thông tin màn hình MyPage API mới:
  registerOrUpdateMember: (
    ...
    return `${URL_DOMAIN}/api/v1/app/${APP_ID1}/member/registerOrUpdateMember?${
      phoneNumber ? `phoneNumber=${phoneNumber}` : ''
    }${password ? `&password=${password}` : ''}&gender=${gender}${
      birthday ? `&birthday=${birthday}` : ''
    }${zipCode ? `&zipCode=${zipCode}` : ''}${
      listFavoriteCategory
        ? `&listFavoriteCategory=${listFavoriteCategory}`
        : ''
