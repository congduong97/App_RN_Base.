...
const UpdateLinkingCard = () => {
  const navigation = useNavigation();
  const route = useRoute();
  const currentMemberCode = route.params.memberCode;
  const data = useRef({
    memberCode: '',
    pinCode: '',
  });
  ...
  const onValidate = () => {
    ...
    if (memberCode === currentMemberCode) {
      error.current = STRING.member_code_was_linked;
      forceUpdate();
      return false;
    }
    if (pinCode.length == 0) {
  ...
  const onLinkingCard = async () => {
    ...
    if (validate) {
      ...
      if (response.success) {
        ...
        navigation.navigate(keyNavigation.CONFIRM_MESS, {key: 'UPDATE_CARD'});
      } else {
        if (
          response &&
  ...
  return (
    <View style={{flex: 1}}>
      <AppHeader title={'KOMECA変更・削除'} leftGoBack />
          ...
          <AppText
            onPress={() => {
              navigation.navigate(keyNavigation.MY_PAGE_PASS_CONFIRM, {
                keyActiveButton: 'REMOVE_LINKINGCARD',
              });
            }}
            style={{
              minHeight: 25,
...
export default UpdateLinkingCard;
