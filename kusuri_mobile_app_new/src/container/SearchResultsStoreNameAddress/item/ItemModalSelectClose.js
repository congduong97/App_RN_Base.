...
export default class ItemModalSelectClose extends PureComponent {
    ...
    componentDidMount() {
        setNameCheck.onChange('placehoderCLose', data => {
            if(data.data.type==='SetCheckClose'&& data.data.checkCity){
                this.closeOptions();
             }
        })
    ...
    componentWillUnmount(){
        setNameCheck.unChange('placehoderCLose');
    }
    enableModal = () => {
    ...
    renderContainer = () => {
        // const { data,isLoadingRefreshCloseTime} = this.props;
