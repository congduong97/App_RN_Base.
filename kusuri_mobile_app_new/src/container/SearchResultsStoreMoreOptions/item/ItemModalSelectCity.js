...
export default class ItemModalSelectCity extends PureComponent {
    constructor(props) {
        super(props);
        const dataCity = CheckBoxNameService.get();
        this.state = {
            isModalVisible: false,
            placehoder: dataCity.city.name ? dataCity.city.name : STRING.placehoderCity,
            loadingCity: false,
            errCity: false,
            dataListCities: [],
    ...
    componentDidMount() {
        setNameCheck.onChange('placehoderCity', data => {
            if (data.data.type === 'setplacehoder') {
                this.closeOptions();
            }
        })
        this.getListCities();
    ...
    componentWillUnmount() {
        setValueAllItem.unChange('placehoderCity')
    }
    enableModal = () => {
    ...
    setValueSelect = (item) => {
        ...
        setValueAllItem.set({ type: 'setIdCitySearchMoreOptions', item })
        if (valueText) {
    ...
    closeOptions = () => {
        ...
        setNameCheck.set({ type: 'checkDistrict', idCheck: true })
    }
