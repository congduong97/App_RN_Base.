...
import models, { Gender } from "../../../models";
...
export default function PopupChoiceView(props) {
  ...
  if (typeDialog === PopupType.ShowHealthFacilities) {
    ...
    if (dataDialog.length > 0) {
      dataDialog.forEach(
        (item) =>
          (item.isChecked = item.id === dataSelected?.[keyCheck] ? true : false)
      );
    }
  } else if (typeDialog === PopupType.ShowMedicalServices) {
  ...
  } else if (typeDialog === PopupType.ShowMedicalSpecialities) {
    titleDialog = "Chọn Chuyên khoa khám";
    dataDialog = medicalSpecial || [];
    if (dataDialog.length > 0)
      dataDialog.forEach(
        (item) =>
          (item.isChecked = item.id === dataSelected?.[keyCheck] ? true : false)
      );
  } else if (typeDialog === PopupType.ShowGender) {
    titleDialog = "Chọn giới tính";
    dataDialog = Gender;
    dataDialog.forEach(
      (item) =>
        (item.isChecked =
          item.code?.toUpperCase() === dataSelected?.[keyCheck]?.toUpperCase()
            ? true
            : false)
    );
  } else if (typeDialog === PopupType.ShowRelationships) {
    titleDialog = "Chọn mối quan hệ";
    dataDialog = models.getRelationship();
  ...
  } else if (typeDialog === PopupType.ShowTopic) {
    titleDialog = "Chọn tiêu đề";
    dataDialog = models.getTopics();
    if (dataDialog.length > 0) {
      dataDialog.forEach(
        (item) =>
          (item.isChecked = item.id === dataSelected?.[keyCheck] ? true : false)
  ...
  } else if (typeDialog === PopupType.ShowHealthFacilitiesFeedback) {
    titleDialog = "Chọn cơ sở tiếp nhận";
    dataDialog = heathFaciProcessFeeb || [];
    if (dataDialog.length > 0) {
      dataDialog.forEach(
        (item) =>
          (item.isChecked = item.id === dataSelected?.[keyCheck] ? true : false)
  ...
  const handleSearch = () => {
    let data = dataDialog.filter((item) =>
  ...
  return (
    <View style={{ maxHeight: (Dimensions.get("window").height * 5) / 6 }}>
      ...
      <FlatList
        style={{ marginBottom: fontsValue(40) }}
        data={
          typeDialog === PopupType.ShowHealthFacilities ? dataList : dataDialog
        }
