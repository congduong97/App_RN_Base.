...
export default function MyComponent(props) {
  const { onPress, dateSelected, healthFacilityId } = props;
  ...
  useEffect(() => {
    ...
    return () => { };
  }, [dateSelected, healthFacilityId]);

  const timeOptionPeopleRegistered = async (isMoring = true) => {
    var dataSelect = [];
    if (isMoring) {
      let dataResponse = await API.requestHospitalWorkingTime(dispatch, {
      ...
      if (dataResponse.length > 0) {
        ...
        dataSelect = dataAvailable.filter(
          (item) => item.title == dataResponse[0].time
        );
      }
    } else {
      let dataResponse = await API.requestHospitalWorkingTime(dispatch, {
    ...
    if (dataSelect.length > 0) {
      selectItem(dataSelect[0], isMoring);
    }
  };
  ...
  const selectItem = (itemData, isMorning) => {
    itemData.isMorning = isMorning;
  ...
  const renderItem = (item, index, isMorning) => {
  ...
  return (
    <View style={{ paddingHorizontal: 16 }}>
      <View style={styles.stHeaderSession}>
        ...
        {allowTimeDefault === 1 && (
          <Text
            onPress={timeOptionPeopleRegistered}
            style={styles.stTextOption}
          >
            {"Tùy chọn thời gian"}
          </Text>
        )}
      ...
      <View style={styles.stHeaderSession}>
        ...
        {allowTimeDefault === 1 && (
          <Text
            onPress={() => {
              timeOptionPeopleRegistered(false);
            }}
            style={styles.stTextOption}
          >
            {"Tùy chọn thời gian"}
